            ...
            <div class="card-body">
                <div class="content-wrapper">
                    ...
                    <div class="container">
                        {{-- search --}}
                        <div class="col-auto">
                            <form action="{{ route('suratdisposisi.suratmasuk') }}" method="GET" class="d-flex">
                                <input type="text" name="search" class="form-control me-2" placeholder="Search"
                                    value="{{ request('search') }}">

                                <select name="perihal" class="form-select me-2">
                                    <option value="">-- Filter Perihal --</option>
                                    @foreach ($perihal as $item)
                                        <option value="{{ $item }}"
                                            {{ request('perihal') == $item ? 'selected' : '' }}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="px-6 py-2">Lampiran</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($suratdisposisi as $index => $item)
                                        <tr>
                                            <th class="px-6 py-4">{{ $index + $suratdisposisi->firstItem() }}</th>
                                            <td class="px-6 py-4">{{ $item->nmrsurat }}</td>
                                            <td class="px-6 py-4">{{ $item->tglterima }}</td>
                                            <td class="px-6 py-4">{{ $item->mastercabang->cabang }}</td>
                                            <td class="px-6 py-4">{{ $item->perihal }}</td>
                                            <td class="px-6 py-4">{{ $item->disposisi }}</td>
                                            <td class="border textmid">
                                                @if ($item->lampiran)
                                                    <!-- Periksa nama kolom yang sesuai -->
                                                    <a href="{{ asset('lampiran/' . $item->lampiran) }}"
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                @else
                                                    ...
                                                    @if (Auth::user()->hakakses('pimpinan'))
                                                        <form action="{{ route('updateStatus', $item->id) }}"
                                                            method="POST">
                                                            @csrf
